...
async function main() {
  ...
  */
  ...
  for(let i = 0; i < numberOfMints; i++){
    ...
    //console.log("svg: ", svg);
    fs.writeFileSync(outDir+`SVG/path${i}.svg`, svg);
    await downloadPNG(svg, i)

    // Get metadata
    let uri = await contract.tokenURI(i)
    //console.log("uri: ", uri);
    fs.writeFileSync(outDir+`JSON/path${i}.json`, uri);
...
{
  let resizewidth = 500;
  let resizedest = outDir+"PNG/path" + tokenId + ".png";
